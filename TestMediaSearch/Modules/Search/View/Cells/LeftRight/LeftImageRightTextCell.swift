...
final class LeftImageRightTextCell: UITableViewCell, NibReusable, ListItem {
    ...
    func configure(with viewModel: ListItemViewModel, delegate: ListItemDelegatable?) {
        guard let vm = viewModel as? LeftImageRightTextViewModel else{
            return
        }
